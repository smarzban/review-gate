...
export function spawnBounded(
  ...
  opts: { cwd: string; timeoutMs: number; maxBytes: number; byteCap: "abort" | "truncate"; graceMs?: number; signal?: AbortSignal },
): Promise<SpawnResult> {
  ...
  return new Promise((resolve) => {
    ...
    child.stdout.on("data", (d: Buffer) => {
      ...
      if (bytes > opts.maxBytes) {
        if (opts.byteCap === "abort") { byteAbort = true; killGroup("SIGKILL"); return finish({}); }
        truncated = true; killGroup("SIGKILL"); return; // truncate: stop the child, keep what we have, settle on close
      }
      out += d.toString();
    });
