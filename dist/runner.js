...
const spawnCall = (backend, model, prompt, repoDir, timeoutMs) => new Promise((resolve, reject) => {
    ...
    child.stdout.on("data", (d) => {
        ...
        if (bytes > MAX_OUTPUT_BYTES) {
            ...
            child.kill("SIGKILL");
            return reject(new Error(`${backend}(${model}) output exceeded ${MAX_OUTPUT_BYTES} bytes`));
        }
        out += d;
    });
    child.stderr.on("data", (d) => { if (err.length < MAX_OUTPUT_BYTES)
    ...
    child.on("close", (code) => {
        ...
        if (timedOut)
            return reject(new Error(`${backend}(${model}) timed out after ${timeoutMs}ms`));
